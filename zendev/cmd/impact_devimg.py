...
def impact_devimg(args, env):
    ...
    if not args.impact_image:
        error('missing required resource "impact-image" argument for building impact-devimg')
        return
    impact_src_image = args.impact_image
    impact_dst_image = 'zendev/impact-devimg'
    container_id = 'impact_devimg_' + uuid.uuid1().hex
    pom_file = env().srcroot.strpath + '/github.com/zenoss/impact-server/model-adapters-common/pom.xml'
...
def add_commands(subparsers):
    impact_devimg_parser = subparsers.add_parser('impact_devimg',
                                                 help='Build a developer image for Zenoss Impact ')
    impact_devimg_parser.add_argument('--impact-image',
                                      help="In an impact-devimg build, the image to use as a source to build new image on top of it")
