...
def devimg(args, env):
    ...
    All of the business logic for building a devimg is defined in the devimg
    subdirectory of the product-assembly repo.  All this command needs to do
    is invoke the make in product-assembly/devimg with the right arguments,
    which are mostly environment variables.
    See product-assembly/devimg/makefile for more details.

    For the zendev user, the primary choice is which set of zenpacks (if any)
    to include in the image. The options are: none, all of those specified
    for one of the products defined in product-assembly (e.g. core, resmgr,
    etc), the set of zenpacks defined in a custom zenpacks.json file, or
    simply an adhoc list defined on the zendev command line.
    """
    environ = env()
    ...
    if args.product:
        ...
        # This is just a simple sanity check to avoid building from
        # subdirectories of product-assembly which are NOT actually
        # product directories.
        zenpackManifestFile = targetDir.join("zenpacks.json")
        if not zenpackManifestFile.check():
            error(
                "Target product '%s' does not appear to be a valid "
                "product. Could not find %s"
                % (args.product, zenpackManifestFile.strpath)
            )
            sys.exit(1)
...
def add_commands(subparsers):
    devimg_parser = subparsers.add_parser(
        "devimg",
        help="Build a developer image of Zenoss containing either no "
        "zenpacks, the set of zenpacks matching one of the standard "
        "products (core, resmgr, etc), or a custom set of zenpacks",
    )
    devimg_parser.add_argument(
        "-c",
        "--clean",
        action="store_true",
        default=False,
    ...
    zenpacks_parser = devimg_parser.add_mutually_exclusive_group(
        required=False
    )
    zenpacks_parser.add_argument(
        "-p",
        "--product",
        metavar="PRODUCT",
        required=False,
        help="Name of a Zenoss product that defines the set of zenpacks "
        "copied into the image; e.g. core, resmgr, etc",
    )
    zenpacks_parser.add_argument(
        "-f",
        "--file",
        help="Path to a zenpacks.json file that defines the set of zenpacks "
